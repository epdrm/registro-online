...
/** Opções de turma = séries reais cadastradas em Cursos, agrupadas por curso (com o eixo no rótulo). */
function OpcoesTurma({ eixos, cursos, series }: { eixos: Eixo[]; cursos: Curso[]; series: Serie[] }) {
  if (series.length === 0) {
    return <option value="" disabled>Nenhuma turma cadastrada — cadastre em Cursos</option>
  }
  return (
    <>
      {cursos.map((c) => {
        const seriesDoCurso = series.filter((s) => s.curso_id === c.id)
        if (seriesDoCurso.length === 0) return null
        const eixo = eixos.find((e) => e.id === c.eixo_id)
        return (
          <optgroup key={c.id} label={eixo ? `${eixo.nome} · ${c.nome}` : c.nome}>
      ...
      <form onSubmit={aoEnviar} className="flex flex-col gap-4 rounded-xl border border-border bg-bg p-5 sm:p-6">
        ...
        <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2">
          ...
          {papel === 'professor_diretor' && (
            <label className="flex flex-col gap-1.5">
              <span className="text-[13px] font-semibold">Turma responsável</span>
              <select required value={turmaResponsavelId} onChange={(e) => setTurmaResponsavelId(e.target.value)}
                className="tap-target rounded-lg border border-border bg-bg px-3.5 text-[14px] outline-none focus:border-green">
                <option value="">Selecione…</option>
                <OpcoesTurma eixos={eixos} cursos={cursos} series={series} />
              </select>
            </label>
          )}
        </div>
                ...
                <div className="flex flex-col gap-3">
                  <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2">
                    ...
                    {editPapel === 'professor_diretor' && (
                      <label className="flex flex-col gap-1.5">
                        <span className="text-[13px] font-semibold">Turma responsável</span>
                        <select required value={editTurmaResponsavelId} onChange={(e) => setEditTurmaResponsavelId(e.target.value)}
                          className="tap-target rounded-lg border border-border bg-bg px-3.5 text-[14px] outline-none focus:border-green">
                          <option value="">Selecione…</option>
                          <OpcoesTurma eixos={eixos} cursos={cursos} series={series} />
                        </select>
